 ...
 * DPH připomínka (sama se zkratuje mimo ~20. den), upozornění na hlídané termíny
 * (revize, servis, administrativa) a nakonec denní souhrn — tak jde i DPH připomínka
 * a připomínka termínu v režimu „souhrn" ven týž den. Všechny kroky jsou idempotentní;
...
$tasks = $run('app:tasks:remind');
$digest = $run('app:notifications:digest');

foreach ([$recalculate, $identifiedPerson, $remind, $tasks, $digest] as $code) {
    if ($code !== 0) {
        exit($code);
    }
}
